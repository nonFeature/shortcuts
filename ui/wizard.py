...
def build_wizard_step1(plugin):
    ...
    items.append(Text(
        text=_s("next"),
        accent=True,
        create_sub_fragment=lambda: build_wizard_step2(plugin)
    ))
    return items
    ...
    def _build():
        ...
        if type_i == 1:  # open_settings
            ...
            if subs:
                ...
                items.append(Selector(
                    key="__wiz_subfragment",
                    text=_s("sub_fragment"),
                    items=sub_titles,
                    default=0
                ))
            ...
            items.append(Text(
                text=_s("next"),
                accent=True,
                create_sub_fragment=lambda: build_wizard_step_customize(plugin, pids, "open_settings", sub_keys=sub_keys)
            ))
        elif type_i == 2:  # operate_setting -> choose setting
            items.append(Text(
                text=_s("next"),
                accent=True,
                create_sub_fragment=lambda: build_wizard_step3_setting(plugin, pids)
            ))
        else:  # toggle_plugin -> custom label & icon
            items.append(Text(
                text=_s("next"),
                accent=True,
                create_sub_fragment=lambda: build_wizard_step_customize(plugin, pids, "toggle_plugin")
            ))
...
def build_wizard_step3_setting(plugin, pids):
    def _build():
        ...
        items.append(Text(
            text=_s("next"),
            accent=True,
            create_sub_fragment=lambda: build_wizard_step_customize(plugin, pids, "operate_setting", settings=settings)
        ))
        return items
    return plugin._with_spinner(_build)
...
def build_wizard_step_customize(plugin, pids, stype, sub_keys=None, settings=None):
    ...
    items.append(Text(
        text=_s("create"),
        accent=True,
        on_click=lambda v: wizard_finalize(plugin, pids, stype, sub_keys=sub_keys, settings=settings)
    ))
    return items
