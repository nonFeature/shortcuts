from java.util import Locale

STRINGS = {
    "en": {
        "shortcuts": "Shortcuts",
        "plugins": "Plugins",
        "no_plugins": "No other plugins",
        "settings": "Settings",
        "add_shortcut": "Add shortcut",
        "remove_shortcut": "Remove",
        "edit_shortcut": "Edit shortcut",
        "open_shortcut": "Open",
        "copy_deeplink": "Copy deep link",
        "deeplink_copied": "Deep link copied to clipboard",
        "actions": "Actions",
        "location": "Location",
        "drawer": "Drawer",
        "chat_menu": "Chat menu",
        "message_menu": "Message menu",
        "profile_menu": "Profile menu",
        "both_places": "Drawer + Chat",
        "location_required": "Select at least one menu",
        "location_hint": "Select one or more places for this shortcut",
        "type": "Type",
        "toggle_plugin": "Toggle plugin",
        "open_settings": "Open settings / Sub-page",
        "operate_setting": "Change plugin setting",
        "next": "Next...",
        "create": "Create",
        "shortcut_created": "Shortcut created",
        "shortcut_updated": "Shortcut updated",
        "shortcut_removed": "Shortcut removed",
        "no_settings": "No settings available",
        "toggle_only_warning": "This plugin only supports «Toggle plugin»",
        "error": "Error",
        "plugin_not_found": "Plugin not found",
        "plugin_disabled": "Plugin is disabled",
        "plugin_disabled_open_manager": "Open plugin manager to enable it",
        "quick_access": "Quick access to settings",
        "quick_access_sub": "Add 'Shortcuts' to main menu",
        "select_setting": "Select setting to control",
        "choose_value": "Choose new value",
        "set_value": "Set value",
        "text_value": "Text value",
        "save": "Save",
        "cancel": "Cancel",
        "searching": "Searching...",
        "auto_remove_missing": "Auto-remove missing shortcuts",
        "auto_remove_missing_sub": "Automatically delete shortcuts if plugin is missing",
        "missing_shortcuts_removed": "Removed missing shortcuts",
        "sub_fragment": "Sub-page (Sub-fragment)",
        "root_settings": "Main plugin settings",
        "custom_label": "Shortcut Label",
        "custom_icon": "Icon",
        "choose_icon": "Choose icon",
        "custom_icon_name": "Custom icon name",
        "custom_icon_name_sub": "Enter a drawable name from the client",
        "icon_found": "Icon found",
        "icon_not_found": "Icon not found",
        "icon_selected": "Icon selected",
        "icon_media_settings": "Settings",
        "icon_msg_settings": "Settings gear",
        "icon_msg_folders": "Folders",
        "icon_menu_privacy": "Privacy",
        "icon_msg_notifications": "Notifications",
        "icon_msg_secret": "Lock / Sessions",
        "icon_msg_theme": "Theme",
        "icon_msg_language": "Language",
        "icon_media_share": "Share",
        "icon_msg_stats": "Statistics",
        "icon_msg_info": "Information",
        "icon_msg_work": "Work",
        "icon_msg_channel": "Channel",
        "icon_msg_bot": "Bot",
        "icon_msg_openprofile": "Profile",
        "icon_msg_palette": "Palette",
        "icon_msg_customize": "Customize",
        "icon_msg_media": "Data and storage",
        "icon_msg_autodelete": "Auto-delete",
        "add_deeplink_confirm": "Add shortcut from link?",
    },
    "ru": {
        "shortcuts": "Ярлыки",
        "plugins": "Плагины",
        "no_plugins": "Нет других плагинов",
        "settings": "Настройки",
        "add_shortcut": "Добавить ярлык",
        "remove_shortcut": "Удалить",
        "edit_shortcut": "Редактировать",
        "open_shortcut": "Открыть",
        "copy_deeplink": "Скопировать диплинк",
        "deeplink_copied": "Диплинк скопирован в буфер обмена",
        "actions": "Действия",
        "location": "Расположение",
        "drawer": "Боковое меню",
        "chat_menu": "Меню чата",
        "message_menu": "Меню сообщения",
        "profile_menu": "Меню профиля",
        "both_places": "Боковое меню + Меню чата",
        "location_required": "Выберите хотя бы одно меню",
        "location_hint": "Выберите одно или несколько мест для ярлыка",
        "type": "Тип",
        "toggle_plugin": "Вкл/Откл плагин",
        "open_settings": "Открыть настройки / Саб-фрагмент",
        "operate_setting": "Управление настройкой плагина",
        "next": "Далее...",
        "create": "Создать",
        "shortcut_created": "Ярлык создан",
        "shortcut_updated": "Ярлык обновлен",
        "shortcut_removed": "Ярлык удален",
        "no_settings": "Нет настроек",
        "toggle_only_warning": "Для этого плагина доступно только «Вкл/Откл»",
        "error": "Ошибка",
        "plugin_disabled": "Плагин выключен",
        "plugin_disabled_open_manager": "Открой менеджер плагинов и включи его",
        "quick_access": "Быстрый доступ к настройкам",
        "quick_access_sub": "Добавить 'Ярлыки' в главное меню",
        "select_setting": "Выберите настройку для управления",
        "choose_value": "Выберите новое значение",
        "set_value": "Установить значение",
        "text_value": "Текстовое значение",
        "save": "Сохранить",
        "cancel": "Отмена",
        "searching": "Поиск...",
        "plugin_not_found": "Плагин не найден",
        "auto_remove_missing": "Автоудаление отсутствующих ярлыков",
        "auto_remove_missing_sub": "Автоматически удалять ярлыки, если плагин не найден",
        "missing_shortcuts_removed": "Удалено ярлыков отсутствующих плагинов",
        "sub_fragment": "Саб-фрагмент (Подстраница)",
        "root_settings": "Главный экран настроек",
        "custom_label": "Название ярлыка",
        "custom_icon": "Иконка",
        "choose_icon": "Выбор иконки",
        "custom_icon_name": "Своё имя иконки",
        "custom_icon_name_sub": "Введите имя drawable из клиента",
        "icon_found": "Иконка найдена",
        "icon_not_found": "Иконка не найдена",
        "icon_selected": "Иконка выбрана",
        "icon_media_settings": "Настройки",
        "icon_msg_settings": "Шестеренка",
        "icon_msg_folders": "Папки",
        "icon_menu_privacy": "Приватность",
        "icon_msg_notifications": "Уведомления",
        "icon_msg_secret": "Замок",
        "icon_msg_theme": "Тема",
        "icon_msg_language": "Язык",
        "icon_media_share": "Поделиться",
        "icon_msg_stats": "Статистика",
        "icon_msg_info": "Информация",
        "icon_msg_work": "Работа",
        "icon_msg_channel": "Канал",
        "icon_msg_bot": "Бот",
        "icon_msg_openprofile": "Профиль",
        "icon_msg_palette": "Палитра",
        "icon_msg_customize": "Кастомизация",
        "icon_msg_media": "Данные и память",
        "icon_msg_autodelete": "Автоудаление",
        "add_deeplink_confirm": "Добавить этот ярлык по ссылке?",
    },
}


def _is_ru():
    try:
        return Locale.getDefault().getLanguage() == "ru"
    except Exception:
        return True


def _s(key):
    lang = "ru" if _is_ru() else "en"
    return STRINGS.get(lang, STRINGS["en"]).get(key, key)
